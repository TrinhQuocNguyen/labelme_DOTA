...
from libs.dota_io import DotaReader
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def change_format(self):
        ...
        elif self.usingYoloFormat: self.set_format(FORMAT_DOTA)
        elif self.usingDotaFormat: self.set_format(FORMAT_PASCALVOC)
        ...
        try:
            if self.usingPascalVocFormat is True:
                ...
                if annotationFilePath[-4:] != ".txt":
                    annotationFilePath += TXT_EXT
                print ('Img: ' + self.filePath + ' -> Its txt: ' + annotationFilePath)
                self.labelFile.saveDotaFormat(annotationFilePath, shapes, self.filePath, self.imageData, self.labelHist,
                                                   self.lineColor.getRgb(), self.fillColor.getRgb())
            else:
    ...
    def loadDOTATXTByFilename(self, txtPath):
        if self.filePath is None:
            return
        if os.path.isfile(txtPath) is False:
            return
        ...
        tDotaParseReader = DotaReader(txtPath, self.image)
        shapes = tDotaParseReader.getShapes()
        print (shapes)
        self.loadLabels(shapes)
        self.canvas.verified = tDotaParseReader.verified
